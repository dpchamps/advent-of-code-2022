    ...
    fn parse_line(line: &String) -> Result<Vec<char>, String> {
        line.chars()
            ...
            .map(|register| match register.get(1) {
                ...
                None => Err(format!("Expected value in register, but got nothing. Input line: {}", line))
            })
            .collect()
    }

    pub fn create_vm_registers(input: &[String]) -> Result<Vec<Vec<char>>, String> {
        input.iter().take(input.len()-1).map(StateParser::parse_line).collect::<Result<Vec<Vec<char>>, String>>()
    }
}
...
impl ProgramParser {
    ...
    fn parse_digit(&mut self) -> Result<usize, String> {
        ...
        digit_buffer.parse().map_err(|_| format!("Failed to parse digit {}", digit_buffer))
    }
    ...
    fn parse_digit_directive(&mut self, command: &str) -> Result<usize, String> {
        ...
        for c in command.chars() {
            self.eat(c).ok_or(format!("Expected {} at {}, but found {:?}", c, self.cursor, self.current()));
        }
    ...
    fn parse_move(&mut self) -> Result<Program, String> {
        ...
        Ok(Program::Command(Command::Move(self.parse_digit_directive("move")?)))
    }

    fn parse_from(&mut self) -> Result<Program, String> {
        println!("parsing next from");
        Ok(Program::Command(Command::From(self.parse_digit_directive("from")?)))
    }

    fn parse_to(&mut self) -> Result<Program, String> {
        println!("parsing next to");
        Ok(Program::Command(Command::To(self.parse_digit_directive("to")?)))
    }
...
[N] [C]
[Z] [M] [P]
 1   2   3 ".lines().map(|x| x.to_string()).collect::<Vec<String>>();
